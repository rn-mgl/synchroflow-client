...
const VERIFICATION_STATUS = {
  ...
  rejected: "Sorry, the account was not registered. This could be due to an illegitimate token.",
};
    ...
    <div className="flex flex-col items-center justify-center w-full min-h-screen h-screen bg-white">
      ...
      >
        <div className="w-full flex flex-col items-center justify-center">
          <Image
            src={status === "verifying" ? verifying : status === "verified" ? verified : rejected}
            alt="status"
            className="w-96 saturate-150 drop-shadow-md animate-float"
          />
        </div>

        <p className="text-center">{VERIFICATION_STATUS[status as keyof object]}</p>
